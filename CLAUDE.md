...
- **`FilePromiseDragSource.swift`**: the drag-to-Finder-download mechanism. It's an `NSViewRepresentable` wrapping an `NSView` that implements `NSDraggingSource` + `NSFilePromiseProviderDelegate`, because SwiftUI has no native support for `NSFilePromiseProvider`. Directory drags recurse (`downloadDirectoryRecursively`) by walking `listDirectory` and downloading each file promise-fulfillment-side; single-file drags download directly. Both paths publish an `NSProgress` (`publishFinderProgress`) to drive Finder's download badge on the dropped item — note the destination file/folder must be created *before* `publish()`, since Finder only attaches a published progress to something it can already see and never retries. Multi-selection drag is supported: `handlePan` promises every selected item (via `filesToDrag`) when the grabbed row is part of a multi-item selection, mirroring the context menu's `effectiveSelection`. This is the app's most AppKit-bridging-heavy file — read it fully before modifying drag/download behavior.

**Drag/drop is only partially testable — do not assume it works just because tests pass.** A real Finder drop can't be driven from XCUITest, and modifier-clicks (needed to build a multi-selection) have no clean public XCUITest API on macOS. The `FerriUITests` drag test (`testDraggingARowStartsAFilePromiseDrag`) only asserts that a drag *reaches* `beginDraggingSession`, surfaced via the debug `debug.lastDragStartedFile` notification — it does not verify what actually downloads. Changes to drag behavior (especially selection handling, e.g. the multi-item regression fixed after c466cc8) must be verified by a manual drag-to-Finder check.
